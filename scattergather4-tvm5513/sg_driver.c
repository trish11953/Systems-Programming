...
// getting the file with that file handle
    ...
    {  
        ...
        memcpy(buf, &tbuf[file->pos%SG_BLOCK_SIZE], SG_BLOCK_SIZE/2);
        file->pos += (SG_BLOCK_SIZE/2);
        return SG_BLOCK_SIZE/2;
...
int sgwrite(SgFHandle fh, char *buf, size_t len) {
    ...
    {   
        ...
        // updating block of size 512
        memcpy(&tbuf[file->pos%SG_BLOCK_SIZE], buf, SG_BLOCK_SIZE/2);
        putSGDataBlock(file->blocks[file->pos/SG_BLOCK_SIZE].nid,file->blocks[file->pos/SG_BLOCK_SIZE].bid, &tbuf);
        update(file->blocks[file->pos/SG_BLOCK_SIZE].nid, file->blocks[file->pos/SG_BLOCK_SIZE].bid, &tbuf);
        if (file->pos == file->size)
        {
            file->pos += (SG_BLOCK_SIZE/2);
            file->size += (SG_BLOCK_SIZE/2);
        }
        else
        {
            file->pos += (SG_BLOCK_SIZE/2);
    ...
    {   
        memcpy(&tbuf, buf, SG_BLOCK_SIZE/2);
