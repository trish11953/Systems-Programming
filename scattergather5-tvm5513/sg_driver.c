...
// using this to store node information
typedef struct {
    SG_SeqNum rseq;
    SG_Node_ID nodeid;
    struct Node* next;
...
Node nodes[SG_MAX_BLOCKS_PER_FILE*4]; 
Node *head, *tail;

//function to create a new node
Node* findnode(SG_Node_ID ndeid)
{
    Node* index = head;
    logMessage(SGDriverLevel, "finding node %d", ndeid);
    while(index)
    {
        if (index->nodeid == ndeid)
        { 
            logMessage(SGDriverLevel, "found node %d", index->nodeid);
            return index;
        }
        index = index->next;
    }
...
void appendnode(SG_Node_ID ndeid, SG_SeqNum srem)
{   
    ...
    if (head == NULL)
    {
        head = nde;
        tail = nde;
        //logMessage( LOG_ERROR_LEVEL, "when empty file next = %d ", file->next);  
    }
    ...
    { 
        head->next = nde;
        tail = nde;
        //logMessage( LOG_ERROR_LEVEL, "when one element file next = %d ", file->next); 
    }
    else
    {
        tail->next = nde;
        tail = nde;
        //logMessage( LOG_ERROR_LEVEL, "when more than one element file next = %d ", file->next); 
    }
...
{   
    ...
    else if(liststart->next == NULL)
    { 
        liststart->next = file;
        listend = liststart->next;
        //logMessage( LOG_ERROR_LEVEL, "when one element file next = %d ", file->next); 
    }
    else
    {
...
//logMessage( LOG_ERROR_LEVEL, "after append start = %d, end = %d", liststart, listend);   
return file->handle;
}
...
void printfiles()
{
    ...
    while(index)
    {
        //logMessage(SGDriverLevel, "File name: %s \n", index->path);
        index = index->next;
    }
}
...
{   
    ...
    //logMessage( LOG_ERROR_LEVEL, "rem = %d , blkid = %d ", rem, blkid );
    file->blocks[blknum].nid = rem;
    file->blocks[blknum].bid = blkid;
    //logMessage( LOG_ERROR_LEVEL, "nid = %d , bid = %d ", file->blocks[blknum].nid ,file->blocks[blknum].bid  );
    return 0;
}
////////////////////////////////////////////////////////////////////////////////
//
...
int update(SG_Node_ID nid, SG_Block_ID bid, char *buf)
{
    ...
    // Local and do some initial setup
    logMessage( LOG_INFO_LEVEL, "Initializing local endpoint ..." );
    //finding the node with that node id
    Node* nde = findnode(nid);
    // incrementing rseq
    nde->rseq += 1;
    ...
    if ( (ret = serialize_sg_packet( sgLocalNodeId , // Local ID
                                    ...
                                    sgLocalSeqno,    // Sender sequence number
                                    nde->rseq,  // Receiver sequence number
                                    buf, initPacket, &pktlen)) != SG_PACKT_OK ) {
    ...
    // Send the packet
    rpktlen = SG_BASE_PACKET_SIZE; // will contain data
    if ( sgServicePost(initPacket, &pktlen, recvPacket, &rpktlen) ) {
...
int retrieve(SG_Node_ID nid, SG_Node_ID bid, char *buf)
{
    // Local variables
...
SgFHandle sgopen(const char *path) {
    ...
    if(!file)
    {
        //check append function to see how im making a new file and opening it
        int fileh = append(path);
        return fileh;
    }
    ...
    if(!file->open)
    {
        ...
        file->handle = newhandle();
        file->pos = 0;
        //logMessage(SGDriverLevel, "opened file fh=%d", file->handle );
        return file->handle;
    }
...
// getting the file with that file handle
    ...
    if (!file)
        return -1;
    logMessage( SGDriverLevel, "file pos = %d, file size %d", file->pos, file->size);

    char tbuf[SG_BLOCK_SIZE];
    char *cachebuf;
    if (file->pos < file->size)
    {  
        //logMessage( SGDriverLevel, "blknum= %d nid = %d, blk = %d", file->pos/SG_BLOCK_SIZE, file->blocks[file->pos/SG_BLOCK_SIZE].nid, file->blocks[file->pos/SG_BLOCK_SIZE].bid);
        cachebuf = getSGDataBlock(file->blocks[file->pos/SG_BLOCK_SIZE].nid,file->blocks[file->pos/SG_BLOCK_SIZE].bid);
        //nothing in cache
        if(!cachebuf)
        {   
            retrieve(file->blocks[file->pos/SG_BLOCK_SIZE].nid,file->blocks[file->pos/SG_BLOCK_SIZE].bid, &tbuf);
            logMessage(SGDriverLevel,"Reading when !cachebuf tbuf = %s", tbuf);
            putSGDataBlock(file->blocks[file->pos/SG_BLOCK_SIZE].nid,file->blocks[file->pos/SG_BLOCK_SIZE].bid, &tbuf);
        }
        else
        {
            memcpy(&tbuf, cachebuf, SG_BLOCK_SIZE);
            logMessage(SGDriverLevel,"reading when cachebuff tbuf = %s", tbuf);
        }

        // reading the data and then setting pos to what its supposed to be and then return the block size

        memcpy(buf, &tbuf[file->pos % SG_BLOCK_SIZE], SG_RW_SIZE);
        logMessage(SGDriverLevel,"read: buf = %s", buf);
        file->pos += SG_RW_SIZE;
        return SG_RW_SIZE;
...
int sgwrite(SgFHandle fh, char *buf, size_t len) {
    ...
    //unitialised block
    if(file->blocks[file->pos/SG_BLOCK_SIZE].nid != -1 || file->blocks[file->pos/SG_BLOCK_SIZE].bid != -1)
    {   
        cachebuf = getSGDataBlock(file->blocks[file->pos/SG_BLOCK_SIZE].nid,file->blocks[file->pos/SG_BLOCK_SIZE].bid);
        //nothing in cache
        if(!cachebuf)
        { 
            retrieve(file->blocks[file->pos/SG_BLOCK_SIZE].nid,file->blocks[file->pos/SG_BLOCK_SIZE].bid, &tbuf);
            logMessage(LOG_ERROR_LEVEL," write retrieve when not cachebuff tbuf = %s", tbuf);
            //putSGDataBlock(file->blocks[file->pos/SG_BLOCK_SIZE].nid,file->blocks[file->pos/SG_BLOCK_SIZE].bid, &tbuf);
        }
        else
        {   
            memcpy(&tbuf, cachebuf, SG_BLOCK_SIZE);
            logMessage(LOG_ERROR_LEVEL,"write retrieve if cachebuf tbuf = %s", tbuf);
        }
        
        // updating block of size 256
        memcpy(&tbuf[file->pos%SG_BLOCK_SIZE], buf, SG_RW_SIZE);
        putSGDataBlock(file->blocks[file->pos/SG_BLOCK_SIZE].nid,file->blocks[file->pos/SG_BLOCK_SIZE].bid, &tbuf);
        logMessage(LOG_ERROR_LEVEL,"write updated tbuf = %s", tbuf);
        update(file->blocks[file->pos/SG_BLOCK_SIZE].nid, file->blocks[file->pos/SG_BLOCK_SIZE].bid, &tbuf);
        if (file->pos == file->size)
        {
            file->pos += SG_RW_SIZE;
            file->size += SG_RW_SIZE;
        }
        else
        {
            file->pos += SG_RW_SIZE;
    ...
    {   
        memcpy(&tbuf, buf, SG_RW_SIZE);
        // creating new block of size 1024
        logMessage(LOG_ERROR_LEVEL,"write allocated tbuf = %s", tbuf);
        allocateandwrite(file, file->pos/SG_BLOCK_SIZE, &tbuf);
        cachebuf = getSGDataBlock(file->blocks[file->pos/SG_BLOCK_SIZE].nid,file->blocks[file->pos/SG_BLOCK_SIZE].bid);
        //logMessage( LOG_ERROR_LEVEL,"nid = %d, bid = %d", file->blocks[file->pos/SG_BLOCK_SIZE].nid, file->blocks[file->pos/SG_BLOCK_SIZE].bid);
        putSGDataBlock(file->blocks[file->pos/SG_BLOCK_SIZE].nid,file->blocks[file->pos/SG_BLOCK_SIZE].bid, &tbuf);
        file->size += SG_RW_SIZE;
        file->pos += SG_RW_SIZE;
        return len;
